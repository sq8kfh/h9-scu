 ...
 *
 * Created by SQ8KFH on 2017-08-04.
 *
...
static void main_screen_refresh(void);
static void menu_screen_refresh(void);
static void ui(void);
static void backlight_print(void);
static void backlight_change(uint8_t kbr);
static void switch_id_print(void);
static void switch_id_change(uint8_t kbr);
static void power_switch_id_print(void);
static void power_switch_id_change(uint8_t kbr);
static void num_of_ant_print(void);
static void num_of_ant_change(uint8_t kbr);
...
int main(void) {
    ...
    LCD_Initalize();
	LCD_Home();
    LCD_WriteText("Antennas switch ");
	LCD_GoTo(0, 1);
    char buf[17];
    snprintf(buf, 17, "v%d.%d  by SQ8KFH", VERSION_MAJOR, VERSION_MINOR);
    LCD_WriteText(buf);
		    ...
		    if (cm.type == H9MSG_TYPE_GET_REG) {
				...
				switch (cm.data[0]) {
					case 10:
						...
						break;
					case 11:
                        set_switch_id(((cm_res.data[1] << 8) | cm_res.data[0]) & ((1<<H9MSG_DESTINATION_ID_BIT_LENGTH) - 1));
						...
						break;
					case 12:
                        set_number_of_antennas(cm.data[1]);
...
void switch_id_change(uint8_t kbr) {
    if (kbr & KB_CW_BUTTON) {
        ++switch_node_id;
        switch_node_id = switch_node_id & ((1<<H9MSG_DESTINATION_ID_BIT_LENGTH) - 1);
    }
...
void power_switch_id_change(uint8_t kbr) {
    if (kbr & KB_CW_BUTTON) {
        ...
        power_switch_node_id = power_switch_node_id & ((1<<H9MSG_DESTINATION_ID_BIT_LENGTH) - 1);
    }
    else if (kbr & KB_CCW_BUTTON) {
        --power_switch_node_id;
        power_switch_node_id = power_switch_node_id & ((1<<H9MSG_DESTINATION_ID_BIT_LENGTH) - 1);
